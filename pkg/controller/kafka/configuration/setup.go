...
func SetupConfiguration(mgr ctrl.Manager, l logging.Logger, limiter workqueue.RateLimiter, poll time.Duration) error {
...
func postDelete(_ context.Context, cr *svcapitypes.Configuration, obj *svcsdk.DeleteConfigurationOutput, err error) error {
	if err != nil {
		if strings.Contains(err.Error(), svcsdk.ErrCodeBadRequestException) {
			// skip: failed to delete Configuration: BadRequestException: This operation is only valid for resources that are in one of the following states :[ACTIVE, DELETE_FAILED]
			return nil
		}
		return err
	}
