...
func (e *external) Observe(ctx context.Context, mgd resource.Managed) (managed.ExternalObservation, error) { // nolint:gocyclo
	...
	var response *awsr53r.GetResolverRuleAssociationResponse
	var err error
	...
	if err != nil {
		return managed.ExternalObservation{}, awsclient.Wrap(resource.Ignore(r53r.IsResolverRuleAssociationNotFoundErr, err), errDescribe)
	}

	observed := response.ResolverRuleAssociation
	current := cr.Spec.ForProvider.DeepCopy()

	cr.SetConditions(xpv1.Available())

	cr.Status.AtProvider = r53r.GenerateRoute53ResolverObservation(*observed)
