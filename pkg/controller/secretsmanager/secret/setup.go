...
func (e *hooks) lateInitialize(cr *svcapitypes.SecretParameters, resp *svcsdk.DescribeSecretOutput) error {
	payload, err := e.getPayload(context.TODO(), cr)
	...
	if payload == nil {
		...
		sc := &corev1.Secret{}
		sc.ObjectMeta.Name = ref.Name
		sc.ObjectMeta.Namespace = ref.Namespace
...
func getAWSSecretData(ref *svcapitypes.SecretReference, s *svcsdk.GetSecretValueOutput) (map[string][]byte, error) { // nolint:gocyclo
	...
	if v, exists := parsed[awsclients.StringValue(ref.Key)]; exists {
		payload[awsclients.StringValue(ref.Key)] = []byte(v)
	} else {
...
func getSecretRef(cr *svcapitypes.SecretParameters) (*svcapitypes.SecretReference, error) {
	switch {
	case cr.StringSecretRef != nil:
		return cr.StringSecretRef, nil
	case cr.BinarySecretRef != nil:
		return cr.BinarySecretRef, nil
	}
